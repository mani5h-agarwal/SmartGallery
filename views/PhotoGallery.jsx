...
// Generate or retrieve user_id (store persistently)
...
const uploadImage = async (uri) => {
  try {
    ...
    // Get user_id (either existing or new)
...
// New function to get list of uploaded images
...
// New function to save multiple uploaded images at once
...
const PhotoItem = memo(({ 
  ...
  const rotateAnim = useRef(new Animated.Value(0)).current; // New animation for rotation effect
  
  useEffect(() => {
    // Start animation with a slight delay based on index for staggered effect
    const delay = index * 40; // Slightly faster than the original
    Animated.parallel([
      ...
      // Add a subtle rotation animation for more dynamic entry
  ...
  useEffect(() => {
    Animated.spring(selectAnim, { // Changed to spring for more bounce
  ...
  const handlePress = () => {
    // If already uploaded, don't allow selection in selection mode
    if (isSelectionMode && isUploaded) {
      return;
    }
    
    if (isSelectionMode) {
      // Add a bounce effect when selecting
      Animated.sequence([
        Animated.timing(scaleAnim, {
          toValue: 0.9,
          duration: 100,
    ...
    // Add a short scale animation on press with spring bounce
    Animated.sequence([
      Animated.timing(scaleAnim, {
        toValue: 0.9,
        duration: 100,
  ...
  const handleLongPress = () => {
    // If already uploaded, don't allow selection
    if (isUploaded) {
      return;
    }
    
    // Add tactile feedback with a pulse animation
    Animated.sequence([
      Animated.timing(scaleAnim, {
        toValue: 0.8,
        duration: 100,
    ...
    // IMPORTANT: Immediately select the item without any animation delays
    // This ensures the selection happens right away
    onSelect(item);
  };

  // Calculate rotation based on index for a slight stagger effect
...
const PhotoGallery = () => {
  // Optimize animation performance at the component level
if (Platform.OS === 'android') {
  ...
  const instructionAnim = useRef(new Animated.Value(0)).current; // New animation for instructions

  // Reference to FlatList to maintain scroll position
  const flatListRef = useRef(null);
  
  // Track scroll position
  const scrollPosition = useRef(0);

  // Pulse animation for upload complete
  const uploadDoneAnim = useRef(new Animated.Value(1)).current;

  // Animation for selection mode toggle
  ...
  useEffect(() => {
    // Show instructions with a fade-in & slide-up after initial load
  ...
  useEffect(() => {
    ...
    if (selectedPhotos.length === 0) {
      ...
      // Enter selection mode if photos are selected and not already in selection mode
      setIsSelectionMode(true);
    }
  }, [selectedPhotos, isSelectionMode]);

  // Handle upload completed message with animation
  useEffect(() => {
    if (showUploadDone) {
  ...
  const handleSearch = async () => {
    ...
    // Search animation - bounce the search bar
    Animated.sequence([
      Animated.timing(searchBarAnim, {
        toValue: 5,
        duration: 100,
  ...
  const toggleSelectAll = useCallback(() => {
    ...
    // Filter out uploaded photos from the selection process
  ...
  const handlePhotoPress = useCallback((photo) => {
    ...
    // Set the photo immediately - this is important to do first
    setSelectedPhoto(photo);
    
    // Use spring for the scale animation with optimized parameters
    Animated.parallel([
      ...
      {isLoading ? (
        ...
        <Animated.View style={{ 
          ...
          <FlatList
            ...
            // This is important: don't change FlatList key when changing modes
            // removeClippedSubviews={false}
            // Maintain exact component instances to prevent re-rendering
